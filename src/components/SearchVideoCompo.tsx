...
import { searchVideos } from '@/domain/video/api/search.service';

export default function SearchVideoCardComponent({ query, vertical = false }: { query: string, vertical?: boolean }) {
  ...
  const fetchVideos = useCallback(async (isRefresh = false) => {
    try {
      ...
      const result = await searchVideos(query, 1, 10);
      setVideos(result);
    } catch (err: any) {
